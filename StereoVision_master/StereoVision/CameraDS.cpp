...
IplImage* CCameraDS::QueryFrame()
{
	...
	if (size != m_nBufferSize)
	{
		...
		/*if (m_pFrameold)
			cvReleaseImage(&m_pFrameold);
		m_pFrameold = cvCreateImage(cvSize(m_nWidth, m_nHeight), IPL_DEPTH_8U, 1);*/
	}
	...
	{
		m_pFrame->imageData[i3 + 0] = m_pFrameold->imageData[i];
		m_pFrame->imageData[i3 + 1] = m_pFrameold->imageData[i];
		m_pFrame->imageData[i3 + 2] = m_pFrameold->imageData[i];
